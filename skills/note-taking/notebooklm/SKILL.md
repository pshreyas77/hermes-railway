...
### Windows: Browser Path Discovery & Edge Fallback

On Windows, `notebooklm login --browser chrome` may fail if Chrome is not at the expected path
(`C:\Users\<user>\AppData\Local\Google\Chrome\Application\chrome.exe` — common on portable installs).

**Find Chrome first:**
```bash
where chrome 2>/dev/null || find "/c/Users/$USER" -name "chrome.exe" -maxdepth 8 2>/dev/null | head -3
```

**If Chrome is missing but Edge exists (default on Windows 10/11):**
```bash
notebooklm login --browser msedge
```
Edge is almost always already signed in, so the auth succeeds immediately. This is the most reliable Windows auth path.

**If Playwright's bundled Chromium crashes (first run, or Python 3.13+ without `rookiepy`):**
```bash
playwright install chromium
notebooklm login
```
Playwright auto-downloads its own Chromium on first run if not found. On Python 3.13+, skip the `[cookies]` extra — only `[browser]` is needed for the login flow.

> ⚠️ **Windows/MSYS environments:** Shell commands like `start url` or `xdg-open` open URLs in a background tab, not the foreground active window. Always prefer `notebooklm login --browser msedge` for headed auth.
...
**Piping `ask --json` for clean output extraction:** When the answer field
contains markdown-formatted rich text (tables, lists, formatting) and you need
the raw content without JSON wrapper noise, pipe through Python:

```bash
notebooklm ask "your question" --json 2>&1 | python -c "
import sys, json
raw = sys.stdin.read()
try:
    d = json.loads(raw)
    print(d.get('answer', raw[:3000]))
except:
    print(raw[:3000])
"
```

This strips the JSON envelope and prints only the `.answer` field — the
notebook's synthesized response with all formatting intact. Use 2>&1 to
capture stderr (progress messages) alongside stdout so the JSON parse doesn't
break on mixed streams. For long answers, increase the fallback slice limit.
...
2. Start deep research (non-blocking):
   ...
   ```
3. **Spawn a background subagent** to wait and import.
   - **In Hermes Agent:** use `delegate_task`:
     ```
     delegate_task(
       goal="Wait for research in notebook {notebook_id} to complete and import sources.
             Use: notebooklm research wait -n {notebook_id} --import-all --timeout 1800
             Report how many sources were imported.",
       context="Notebook ID: {notebook_id}, Title: {title}"
     )
     ```
   - **In Claude Code:** use the `Task` tool with `subagent_type="general-purpose"`.
   - The subagent blocks on `research wait --import-all` (up to 30 min for deep mode).
4. Main conversation continues while the subagent waits.
5. When subagent completes, sources are imported — proceed to chat or generation.

**What NOT to do:** Do not run `research wait` in the foreground terminal call —
it blocks the main session for 15-30+ minutes. Always background it via
`delegate_task` (Hermes) or equivalent. The `--no-wait` flag on `source
add-research` is what makes this pattern possible — it returns immediately
with a task ID.
...
### Wikipedia URLs Are Blocked by NotebookLM
`source add "https://en.wikipedia.org/wiki/Theyyam"` fails with "URL is invalid or inaccessible".
NotebookLM's crawler does not reliably process Wikipedia article pages.
**Workaround:** Use `notebooklm source add-research "Theyyam Kerala folk religion"` instead — the research
feature fetches from search results and bypasses the direct-URL block. For all web-based research,
prefer `source add-research` over `source add URL`.

### Source Count Desync (CLI Shows 0, Backend Has Sources)
Running `source list --json` sometimes returns `{"sources": [], "count": 0}` even when the notebook
has sources (confirmed by a subsequent add-failing with "51/50 sources"). The CLI's cached source
list is out of sync with NotebookLM's backend state.
**Workaround:** Do not rely on `source list` for accuracy. Use `notebooklm artifact list` and
`notebooklm research status` to infer state. When hitting source limits, check the error message
—which includes the real count (e.g. "51/50 sources") even when list shows 0.

### Source Limits Per Plan
...
The CLI does not enforce these limits — the NotebookLM backend applies them. When adding a file
fails with a source-limit error, the message includes the real count (e.g. "51/50"). Delete sources
with `source delete-by-title` or `source clean` before adding more. Alternatively, create a new
notebook for a new topic cluster.

### Python 3.13 + Hermes venv pip Issue
On Windows with Python 3.13 in the default path, `pip install notebooklm-py` installs to the
hermes-agent venv (`AppData\Local\hermes\hermes-agent\venv`) but `python3 -c "import pptx"` runs from
Python 3.13 (WindowsApps), which does not have the package. This causes false-negative import
errors. Additionally, `pip install python-pptx` may install to Python 3.14 instead of Python 3.13,
even when run from the Python 3.13 environment.
**Workaround:** Always verify which Python is running: `python3 -c "import sys; print(sys.version)"`.
When packages install to the wrong Python, use `python3 -m pip install package` to target the
correct interpreter explicitly. For PPTX generation from markdown in a skill context, prefer
writing a standalone Python script that imports `notebooklm` and `python-pptx` explicitly, then run
it with the correct Python path.

### Windows Background Terminal Job Control Error
Background processes started with `terminal(background=true)` on Windows/MSYS produce
`bash: no job control in this shell` on stderr — this is harmless and does not indicate failure.
The background process itself runs correctly. This message appears because the MSYS bash environment
does not support POSIX job control primitives. Do not treat it as an error condition.

### Deep Research Does Not Appear in `research status` Immediately
After firing `source add-research --mode deep --no-wait`, `notebooklm research status` may return
`{"research_tasks": []}` for several minutes. This is normal — the research task takes 2-5 minutes
to register in the CLI's view even though it is running on NotebookLM's servers. Do not re-fire the
research or create a new notebook because of this. Wait 5+ minutes before checking status, or
simply delegate a `research wait` subagent and let it poll.

### `source add-research --json` Does Not Exist
Despite `--json` being used throughout the CLI for other commands, `source add-research` does not
accept `--json`. Call it without `--json` and parse the task ID from stdout (format:
`Task ID: ChBlYWQ2N...`). Use `notebooklm research status -n <notebook_id> --json` to check
research status with JSON output.

### Rate Limiting on Generation Is Frequent
Slide deck, infographic, quiz, and audio generation fail with `GENERATION_FAILED` or rate limit
errors regularly (sometimes 30-50% of attempts). This is a Google API constraint, not a bug.
**Workaround:** Always use the subagent pattern for generation. If it fails, retry after 5-10 minutes.
Do not attempt to work around rate limits programmatically — just retry.
